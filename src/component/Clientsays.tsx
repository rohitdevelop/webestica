...
const TestimonialCard = ({ id, text, avatar, name, role, index }) => {
    // Gradient color scheme for borders and text (Cyan/Blue)
    const cardGradient = "from-cyan-400 to-blue-500";
    // Fallback image for safety (as the original paths are commented out)
    const imageFallback = `https://placehold.co/56x56/0f172a/94a3b8?text=${name.charAt(0)}${name.split(' ')[1]?.charAt(0) || ''}`;
    ...
    return (
        ...
        >
            {/* Animated Shadow Border (The 'border shadow' request) */}
    ...
    return (
        <section id="client-says" className="py-24 px-4 bg-black min-h-screen flex items-center justify-center font-sans">
            <div className="max-w-7xl mx-auto w-full">
                ...
                {/* Testimonial Grid Structure (3 then 2 centered - implements 1,2,3 or 4 upper and 5,6,7 lower request for 5 items) */}
                <div className="flex flex-col gap-8">
                    
                    {/* Row 1: Items 1, 2, 3 in a 3-column layout */}
                    ...
                    {/* Row 2: Items 4, 5 in a 2-column layout, centered */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:max-w-4xl mx-auto">
                        {testimonials.slice(3, 5).map((testimonial, index) => (
                            // Index is offset by 3 to maintain the correct staggered animation delay
